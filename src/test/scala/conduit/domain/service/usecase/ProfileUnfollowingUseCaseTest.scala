...
object ProfileUnfollowingUseCaseTest extends KyoTestSuite {
  ...
  override def specSuite: SuiteResult < (Async & Scope) =
    "ProfileUnfollowingUseCase" should {
      "succeed in unfollowing an existing profile" in withDatabase { database =>
        for
          ...
          response    <- ProfileUnfollowingUseCase(database, persistence).apply(request)
        yield assert(
          response.profile.username == targetProf.name &&
          !response.profile.following,
          "Expected profile to be unfollowed",
        )
      }

      "fail to unfollow a non-existent profile" in withDatabase { database =>
        for
          fixtures    <- makeFixtures
          persistence <- makePersistence
          ...
          result <- Abort.run(ProfileUnfollowingUseCase(database, persistence).apply(request))
