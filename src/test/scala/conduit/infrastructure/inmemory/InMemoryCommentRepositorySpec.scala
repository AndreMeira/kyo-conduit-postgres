...
object InMemoryCommentRepositorySpec extends KyoTestSuite:
  ...
  def specSuite: SuiteResult < (Async & Scope) =
    ...
    "InMemoryCommentRepository" should {
      "save and find a comment" in withDatabase { database =>
      ...
      "save assigns distinct DB-generated ids" in withDatabase { database =>
        database.transaction:
          for
            fixtures    <- makeFixtures
            persistence <- makePersistence
      ...
      "exists returns true for a saved comment and false otherwise" in withDatabase { database =>
        database.transaction:
          for
            fixtures    <- makeFixtures
            persistence <- makePersistence
      ...
      "findByArticleId returns comments in creation order" in withDatabase { database =>
        database.transaction:
          for
            fixtures    <- makeFixtures
            persistence <- makePersistence
      ...
      "findByArticleId returns empty for an article without comments" in withDatabase { database =>
        database.transaction:
          for
            fixtures    <- makeFixtures
            persistence <- makePersistence
      ...
      "update changes the comment body" in withDatabase { database =>
        database.transaction:
          for
            fixtures    <- makeFixtures
            persistence <- makePersistence
      ...
      "delete removes a comment" in withDatabase { database =>
        database.transaction:
          for
            fixtures    <- makeFixtures
            persistence <- makePersistence
